...
async function init() {
    try {
        ...
        // Clean data: remove features without names
        countries = data.features.filter(d => d.properties && d.properties.name);
        
        // Populate Autocomplete list
        const datalist = document.getElementById('country-list');
        countries.forEach(c => {
            const option = document.createElement('option');
            option.value = c.properties.name;
            datalist.appendChild(option);
        });

        setMode('daily');
    } catch (err) {
        document.getElementById('message').innerText = "Failed to load map data.";
    }
}
...
function checkGuess() {
    ...
    if (!guessedCountry) {
        document.getElementById('message').innerText = "Unknown country!";
        return;
    }

    guessCount++;
    const dist = calculateDistance(guessedCountry, targetCountry);
    const proximity = Math.max(0, 100 - Math.floor((dist / 15000) * 100)); // 15k km as scale
    const angle = calculateAngle(guessedCountry, targetCountry);

    addGuessToHistory(guessedCountry.properties.name, Math.round(dist), proximity, angle);
    updateHints();

    if (guessName.toLowerCase() === targetCountry.properties.name.toLowerCase()) {
        document.getElementById('message').innerText = "✅ BEAUTIFUL! YOU GOT IT!";
...
function updateHints() {
    ...
    if (guessCount === 1) hintText.innerText = `Continent: ${props.continent || "Global"}`;
    else if (guessCount === 2) hintText.innerText = `Region: ${props.subregion || "N/A"}`;
    else if (guessCount >= 3) hintText.innerText = `Full Name: ${props.formal_en || props.name}`;
}
...
function calculateDistance(c1, c2) {
    ...
    return d3.geoDistance(p1, p2) * 6371;
}
